...
export default function NeuralCapabilities() {
  return (
    <section className="py-20 px-8 md:px-16 lg:px-32 text-center bg-black text-white relative overflow-hidden">
      <h2 className="text-5xl font-extrabold font-['sansation'] text-purple-400 bg-gradient-to-r from-purple-400 to-indigo-500 bg-clip-text text-transparent drop-shadow-lg">
        Next-Gen Neural Capabilities
      </h2>
      <p className="mt-3 text-lg text-gray-300 max-w-3xl mx-auto font-['sansation']">
      ...
      <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {capabilities.map((capability, index) => (
          ...
          >
            ...
            <h3 className="text-xl font-bold font-['sansation'] text-white">{capability.title}</h3>
            <p className="text-gray-400 font-['sansation'] mt-2 text-sm">{capability.description}</p>
          </div>
        ))}
      </div>
    </section>
